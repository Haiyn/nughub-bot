...
export class PermissionService extends Service {
    public hasPermission(user: GuildMember, commandPermissionLevel: number): boolean {
        ...
        this.logger.debug(
            `User has permission level ${userPermissionLevel} while command has permission level ${commandPermissionLevel}. Returning: ${
                userPermissionLevel >= commandPermissionLevel
            }`
        );
